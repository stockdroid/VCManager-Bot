...
async def create_vc(req: Request):
    try:
        await tg_app.invoke(CreateGroupCall(peer=await tg_app.resolve_peer(shared.GROUP_ID), random_id=int((random.random() * 1000) * (random.random() * 1000))))
...
async def record_vc(req: Request):
    ...
    await tg_app.invoke(ToggleGroupCallRecord(call=call_py.full_chat.call, start=start, video=video, video_portrait=False))
